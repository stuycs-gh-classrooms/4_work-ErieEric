...
void setupLand(Land[][] row,int numRows, int numPlots, float density) {
  ...
  for (int i=0; i<numRows; i++) {
    for(int n=0;n<row[i].length;n++){
      ...
      row[i][n] = new Land(n*plotSize, i*plotSize, plotSize, type);
    }//setup loop
  }//setupLand
}
...
void liveFire(Land[][] row) {
  ...
  for(int i=0;i<row.length;i++){
    row[i][0].updateNextState(0);
    for (int n=1; n<row[i].length; n++) {
      row[i][n].updateNextState(row[i][n-1].state);
